...
public class P3D7 extends Application
{
    ...
    // My code gets the color value for each pixel.It then saves those values into 3 seperate integars.
    //Then in order to invert the color is take those values and subtracts them from 255 to get the 
    //inverted values that work on any picture.
    public static void coolEffect()
    {
        ...
        for (int r = 0; r < before1.getHeight(); r++)
        {
            ...
            {
                int SavedRed = before1.getRed(c,r);
                int SavedBlue = before1.getBlue(c,r);
                int SavedGreen = before1.getGreen(c,r);
                int InvRed = 255 - SavedRed;
                int InvBlue = 255 - SavedBlue;
                int InvGreen = 255 - SavedGreen;

                before1.setRed(InvRed,c, r);
                before1.setGreen(InvGreen,c,r);
                before1.setBlue(InvBlue,c,r);
