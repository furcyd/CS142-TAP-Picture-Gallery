...
public class P3D7 extends Application
{
    ...
    public static void coolEffect()
    {
        ...
        before1.loadPicture(beforePic1Name, 585, 725);
        ...
        for (int c = 0; c < before1.getWidth() / 2; c++) {
            for (int r = 0; r < before1.getHeight(); r++) {
                int w = 0; w = before1.getWidth() - 1;
                
                int d = 0; d = before1.getRed(w - c, r); //Saves current values
                int g = 0; g = before1.getGreen(w - c, r);
                int b = 0; b = before1.getBlue(w - c, r);
                
                int rd = 255 - d; //Saves inverted values
                                                
                before1.setRed(255 - before1.getRed(c, r), (w - c), r); //Replaces old with new values
                before1.setGreen(before1.getGreen(c, r), (w - c), r);
                before1.setBlue(before1.getBlue(c, r), (w - c), r);
                
                before1.setRed(rd, c, r); //Finishes replacing old with new values
                before1.setGreen(g, c, r);
                before1.setBlue(b, c, r);           
            } 
        }
    ...
    private static int width = 585;   // update this line as needed
    private static int height = 725;  // update this line as needed
