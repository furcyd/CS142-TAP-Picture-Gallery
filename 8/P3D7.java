...
public class P3D7 extends Application
{
    ...
    /* changes the colors of the image from oranges and reds to blues and purples */
    public static void coolEffect()
    {
        ...
        for (int col = 0; col < before1.getWidth(); col++){
            for (int row = 0; row < before1.getHeight(); row++){
                if (before1.getRed(col, row) > 10){
                    before1.setRed(before1.getRed(col, row) - 100, col, row);
                    before1.setGreen(before1.getGreen(col, row) - 50, col, row);
                    before1.setBlue(before1.getBlue(col, row) + 100, col, row);
                }
            }
        }
    ...
    private static String beforePic1Name = "./pics/before1.png";
